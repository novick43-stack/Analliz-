        ...
        <section className="flex flex-col gap-6 pb-12">
            ...
            <div className="bg-[#f0f9f9] rounded-[2.5rem] p-8 border border-[#e0f2f2]">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {/* Ventas */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-[#81e6d9]">
                        <p className="text-4xl font-black text-gray-900 mb-4">{data?.kpis?.total_orders || 0}</p>
                        <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">ventas</p>
                    </div>

                    {/* Por día */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-[#81e6d9]">
                        <p className="text-4xl font-black text-gray-900 mb-4">{formatCurrency(revenuePerDay)}</p>
                        <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">por día</p>
                    </div>

                    {/* Total facturado */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-[#81e6d9]">
                        <p className="text-4xl font-black text-gray-900 mb-4">{formatCurrency(data?.kpis?.total_revenue)}</p>
                        <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">total facturado</p>
                    </div>

                    {/* Ticket promedio */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-[#81e6d9]">
                        <p className="text-4xl font-black text-gray-900 mb-4">{formatCurrency(data?.kpis?.average_ticket)}</p>
                        <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">por venta (ticket promedio)</p>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex bg-gray-100 p-1 rounded-xl">
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${showRevenue ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                Ingresos
                            </button>
                            <button
                                onClick={() => setShowOrders(!showOrders)}
                                className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${showOrders ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                Pedidos
                            </button>
                        </div>
                        <button onClick={fetchData} className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-blue-600 hover:text-white transition-all cursor-pointer shadow-sm">
            ...
            <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm flex flex-col gap-6">
                ...
                <HeatmapChart data={data?.salesByDayHour || []} loading={loading} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm flex flex-col">
                    ...
                    <div className="mt-6 flex flex-col items-center gap-2">
                        <p className="text-lg font-black text-gray-800">Edad promedio: {data?.averageAge || 0} años</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-[4rem] p-12 border border-gray-100 shadow-2xl flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-8 border-t border-gray-50 pt-8">
                        {(data?.paymentMethods || []).slice(0, 4).map((m: any, i: number) => (
                            <div key={i} className="flex flex-col gap-1">
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }}></div>
                                    <span className="text-[10px] font-black text-gray-500 uppercase truncate">{m.method}</span>
                                </div>
                                <span className="text-sm font-black text-gray-900">{formatCurrency(m.revenue)}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="bg-gray-900 rounded-[5rem] p-16 shadow-2xl text-white">
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5">
                            ...
                            <p className="text-5xl font-black italic">
                                {loading ? "..." : `${((data?.promoImpact?.find((p: any) => p.type === 'Promocional')?.count || 0) / (data?.kpis?.total_orders || 1) * 100).toFixed(1)}%`}
                            </p>
